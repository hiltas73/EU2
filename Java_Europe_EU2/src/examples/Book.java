...
public class Book {
    ...
    public static String capitalize(String word) {
      ...
      else {String [] sentence=word.split(" ");
        String newWord="";
        for(String each: sentence){
         newWord+=each.substring(0,1).toUpperCase()+each.substring(1).toLowerCase()+" ";}
         return newWord.trim();}
    }
    ...
    boolean isTitle=true;
	  String [] check={"a" , "an", "the","and", "but", "for", "nor", "or", "so","yet","at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
    ...
    public void setTitle(String title) {
        ...
        String newWord=capitalize(sentence[0])+" ";
        for(int i=1; i<sentence.length; i++){
        if(!isTitleWord(sentence[i])){
